...
if(generateViews) { 
event("StatusUpdate", ["Geradas as visoes para a classe ${domainClass.name}"]) 
generator.generateViews(domainClass,".") 
} 
if(generateController) { 
event("StatusUpdate", ["Gerados os controles para a classe ${domainClass.name}"]) 
generator.generateController(domainClass,".") 
} 
event("StatusUpdate", ["Finished generation para a classe ${domainClass.name}"]) 
} 
event("StatusFinal", ["Finished generation para a classe"]) 
} 
else { 
event("StatusFinal", ["Nao contem Classe(s) a ser(em) gerada(s)..."]) 
} 
} 
